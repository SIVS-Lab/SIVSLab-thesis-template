...
\begin{itemize}
\item Short space after a ``\,.\,''\\
Usually ``\,.\,'' indicates the end of a sentence, so \TeX\ puts a long space after it.
...
For example: ``\texttt{The cat ran.  Smith et al.\ claim that.}'', yields
...
\begin{center}\fbox{The cat ran.  Smith et al. claim that.}\end{center}
Notice the length of the space before ``claim''.

\item Typesetting of ``et al.'' and ``e.g.''\\
...
\item math: ``\texttt{\textbackslash exp(x)}'' not ``\texttt{exp(x)}''.\\
