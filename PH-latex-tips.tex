...
Here, I also have a writing style suggestion.
The English (or whatever language) should read naturally \textit{without} the citation numbers.
For example,

{\renewcommand{\arraystretch}{1.5}
\begin{tabular}{p{3em}l}
 &  1. Okay:  ``Many methods use this principle [8,9].''\\
 &  2. Okay:  ``As described in by Schneider et al. [13].''\\
 &  3. Nope:  ``As described in [13].''
\end{tabular}
}

The first two look good.  I don't like the third one, probably because
after stripping the number it becomes ``As described in.'' which is ungrammatical.
...
\subsection*{Other Blank Space Related Tips}
\subsubsection*{Avoid the ``H'' option of the \𝚝{figure} environment}
Using the ``H'' option of the \𝚝{figure} environment can cause strange vertical streching of space.
The problem is that ``H'' forces \LaTeX\ to immediately ship out the figure.  If the figure is too big to
fit on the current page; the \LaTeX\ is forced to end the current page even if it does not have a full page
worth of content on it.  The observable symptom of such a page is too much vertical space, because \LaTeX\ has
to stretch out the insufficient contents to span from the top to the bottom of the page.

Instead try perhaps ``ht'' as in\\
